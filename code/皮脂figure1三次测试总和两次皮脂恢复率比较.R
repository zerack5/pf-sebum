...
p3 <- p3+theme(text=element_text(size=10,  family="sans", face = "bold")#times new roma#sans/宋体#mono/雅黑  
  ...
  labs(title ="                                  Sebum recovery rate", x="Testing points ", y="Real CSL (a.u.)")
...
ggsave("三次SRR.tiff",device = "tiff",dpi = 300,path = "C:/Users/a/Desktop/数据分析/皮肤油脂/xlsx/恢复程度")


with(data=t4,
     pairwise.wilcox.test(x=CL,g=label,p.adjust.method = "bonferroni")
)
